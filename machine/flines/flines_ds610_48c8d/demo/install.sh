...
create_demo_gpt_partition()
{
    ...
    if [ "$demo_type" = "DIAG" ] || [ "$demo_type" = "MFG" ]; then
...
EOF
    ...
    demo_grub_entry="FLINES $demo_type"
    cat <<EOF >> $grub_cfg
menuentry '$demo_grub_entry' {
        search --no-floppy --label --set=root $demo_volume_label
        echo    'Loading ONIE FLINES $demo_type kernel ...'
        linux   /demo.vmlinuz $GRUB_CMDLINE_LINUX \$ONIE_EXTRA_CMDLINE_LINUX DEMO_TYPE=$demo_type
        echo    'Loading ONIE FLINES $demo_type initial ramdisk ...'
        initrd  /demo.initrd
}
EOF
